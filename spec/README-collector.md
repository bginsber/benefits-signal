...
What it does each run: reads the source list from `spec/sources.yaml` (`scripts/lib/sources.mjs`) and collects every active source that has a collector (`scripts/lib/collectors.mjs`): the four RSS interpretation sources (Groom, Trucker Huss, Wagner, Word on Benefits); the supplemental commentary feeds (Proskauer's ERISA Practice Center, Benefits Law Advisor, Seyfarth's Beneficially Yours, Morgan Lewis's ML BeneBits, ERIC's policy highlights, and NCCMP's resources feed), which widen the feed toward BenefitsLink-style coverage without joining the curated seven; RSS 2.0, RSS 1.0, and Atom feeds all parse; when Wagner's own feed is blocked by its bot challenge, Wagner items come from a Google News RSS search limited to its domain; Mercer Law & Policy through the search backend its own listing page calls; Segal Compliance News through the listing's JSON endpoint; the Federal Register API for EBSA, IRS, ETA, WHD, HHS, and CMS (with structured comment-deadline and effective-date fields); the DOL and HHS news-release RSS feeds; CourtListener v4 opinion search for Ninth Circuit ERISA, benefit-plan, Taft-Hartley, and parity cases, plus Supreme Court and other-circuit opinions narrowed to welfare-fund terms (docket number, filing date, and opinion PDF kept as structured fields; `COURTLISTENER_TOKEN` is sent when set), and ERISA rulings from the four California district courts read from RECAP dockets (orders, judgments, and findings only), since CourtListener's district-court opinion collection stops in 2019; the weekly Internal Revenue Bulletin's highlights; California DMHC All Plan Letters and press releases; regulations.gov comment periods closing within two weeks, as reminders (`REGULATIONS_GOV_API_KEY` when set, else `DEMO_KEY`); and the California DIR/DAS "What's New" and CAC meeting pages. It stores one JSON document per item under `data/collected/` keyed by URL hash, so re-runs never duplicate; writes `data/run-log.json` with a row for every active source (collected with a count, failed with the error, or skipped with the reason) and a `consecutive_failures` count carried from the previous run, so a source that fails three runs in a row raises a "source silent" warning; and emits `public/collated.xml`, a merged RSS 2.0 feed sorted newest-first, capped at 100 items. The store keeps every document; the collated feed applies `spec/feed-filter.yaml`: every EBSA document except individual prohibited-transaction exemptions that name no welfare, multiemployer, or training plan (EBSA grants many for single-company 401(k) plans); any other Federal Register document (rules and proposed rules included, since the agency list spans all of HHS and IRS) only when its title or abstract names a benefits topic; agency housekeeping (information-collection and OMB-review notices, patent licences, Privacy Act matches) never; no firm news, events, webinars, or Canada-only posts from the commentary sources; Mercer, DOL, HHS, Morgan Lewis, NCCMP, IRB, and comment-deadline items only when they name a benefits topic. Keywords match at the start of a word, so "pension" does not match "suspension". Each item's body is rendered as inline-styled HTML in the newsletter's palette (source kicker, serif headline, date and deadline line, cleaned summary, a single link), which Outlook Classic displays as-is.

Sources with no collector are skipped with a reason in the run log, never silently dropped: EBIA Weekly (email-only — needs the dedicated mailbox) and the DOL Office of Apprenticeship (no newsroom; its releases arrive through the DOL release feed). The CMS/CCIIO guidance index has a collector but is inactive: it carries almost no group-health guidance. Transient fetch failures are retried twice with backoff; a bot challenge such as Wagner's is recorded as a failure, not worked around.
...
## Checking a change before it reaches the live feed

Pushing to a `claude/**` branch runs `.github/workflows/preview.yml`, which never deploys to Pages. It runs the tests, reports on the live feed (`scripts/feed-report.mjs`), probes the candidate feeds in `spec/candidate-feeds.txt` (`scripts/probe-feeds.mjs`: RSS or Atom item counts, items in the last 30 days and how many name a benefits keyword, JSON shape, the feed links an HTML page advertises, and `--dump URL --at REGEX --len N` for copying markup into a parser fixture), runs the branch's collector, and lists what the feed filter kept and dropped (`scripts/filter-report.mjs --kept`). The same scripts work locally after `node scripts/collect.mjs`.
